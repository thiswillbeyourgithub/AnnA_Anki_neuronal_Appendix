 ...
 * `reference_order` either "relative_overdueness" or "lowest_interval". It is the reference used to sort the card before adjusting them using the similarity scores. Default is `"lowest_interval"`. Keep in mind that my relative_overdueness is a reimplantation of the default overdueness of anki and is not absolutely exactly the same.
 * `scoring_weights` a tuple used to adjust the value of the reference order compared to how similar the cards are. Default is `(1, 1.5)`, it means that the algorithm will spread the similar cards farther apart.

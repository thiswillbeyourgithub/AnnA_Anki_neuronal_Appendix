...
* **sBERT is interesting but I'd like to use TFIDF, is this possible?** I initially tried with it and with both combined but it was creating a messy code, you can't cache TFIDF, it slows down the script a lot because SVD does not seem as efficient as PCA, using BERT tokenizer and TFIDF means adding more than 20 parameters that I was not sure about. So I decided to go with only sBERT and it's surprisingly awesome! But if you want to try using TFIDF vectors I reimplemented it.
 ...
 * `TFIDF_stopw_lang` a list of languages used to construct a list of stop words (i.e. words that will be ignored). Default is `["english", "french", "spanish"]`.
